...
SELECT CAST(ROUND(S.LAT_N,4) AS NUMERIC(12,4)) FROM STATION AS S WHERE
 (SELECT count(LAT_N) FROM STATION WHERE LAT_N < S.LAT_N ) = (SELECT count(LAT_N) FROM STATION WHERE LAT_N > S.LAT_N);


--another solution..
   SELECT CAST(ROUND(LAT_N,4) AS NUMERIC(12,4)) FROM (SELECT LAT_N,ROW_NUMBER() OVER(ORDER BY LAT_N ASC) AS NAS, ROW_NUMBER() 
